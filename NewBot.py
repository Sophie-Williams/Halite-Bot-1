...
while True:
    signal.setitimer(signal.ITIMER_REAL, .98)
    ...
    try:
        ...
        for key, value in myPieces.items():
            ...
            if currentSite[1] < currentSite[2]:
                continue
            ...
            if not fastMode:
                ...
                # Prefer obtaining more territory, combine with owned sites if not possible to take over
                if unownedSites:
                    minStrengthSite = min(unownedSites, key=itemgetter(0))
                    lowestStrength = minStrengthSite[0]
                    if currentSite[1] <= lowestStrength:
                        # Let's try to move onto a site we own then
                        logging.info("[{}{}] too weak take new piece, trying to combine...".format(key, currentSite))
                        if ownedSites:
                            for ownedSite in ownedSites:
                                logging.info("[{}{}] checking {}".format(key, currentSite, ownedSite))
                                # Check strength in dict since it is updated as we go
                                loc = getLocation(Location(x, y), direction)
                                strength = mapDict[(loc.x, loc.y)][1]
                                if not movedPiece and currentSite[1] + strength < 255:
                                    logging.info("[{}{}] moving to {}".format(key, currentSite, ownedSite[1]))
                                    moves.append(Move(Location(x, y), ownedSite[1]))
                                    movedPiece = True
                                    continue
                ...
                if not movedPiece and unownedMines:
            ...
            if not movedPiece and unownedPieces:
                ...
                if len(emptyDirection) > 1:
                    ...
                    continue
        frame = frame + 1
        logging.info("moves being submitted: {}".format(moves))
        sendFrame(moves)
    except Exception:
